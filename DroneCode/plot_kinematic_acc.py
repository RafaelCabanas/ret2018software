...
def plot_acceleration3D_vector(time_coordinates):        # input=numpyArray

    fig = plt.figure()
    ax = fig.gca(projection='3d')

    ax.plot(time_coordinates[:,1], time_coordinates[:,2], time_coordinates[:,3], color='red', label='parametric curve + velocity vector + acceleration vecto')
    ...
    for n in range(0,numpy.size(time_coordinates,0)-1,10):
        ax.quiver(time_coordinates[n,1], time_coordinates[n,2], time_coordinates[n,3],time_coordinates[n,7], time_coordinates[n,8], time_coordinates[n,9], color='green')
...
def plot_acceleration3D_vector_ENU(time_coordinates):        # input=numpyArray

    fig = plt.figure(figsize=(20,10))
    ax = fig.gca(projection='3d')

    ax.plot(time_coordinates[:,1], time_coordinates[:,2], time_coordinates[:,3], color='red', label='trajectory TLP + velocity vector + acceleration vector')
...
# Could be used to plot any coordinates in Cartesian system, if you lable columns appropriately.
    print("Select .CSV file to plot. Make sure columns are in the form timestamp, x_local-E, y_local-N, z_local-U, vel_x, vel_y, vel_z, acc_x, acc_y, acc_z")
    ...
    colnames=['timestamp','x_local-E','y_local-N','z_local-U','vel_x','vel_y','vel_z','acc_x', 'acc_y', 'acc_z']
    ...
    plot_acceleration3D_vector_ENU(ENU_forPlot)
